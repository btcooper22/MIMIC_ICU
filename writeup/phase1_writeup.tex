...
\begin{figure}
\centering
  \caption{Flowchart of participants' progress through
  ...
  \begin{tikzpicture}[auto,
    ...
    \matrix [column sep=5mm,row sep=3mm] {
      ...
      \node [block_left] (absent_charts) {Removed patients with absent hospital data (n=69): \\
        \h Chart data (18)\\
    ...
    \begin{scope}[every path/.style=line]
      ...
      \path (hidden_r5) -- (mortality);
      \path (icuprocedure) -- (final);
      \path (hidden_r6) -- (missing_predictors);
      \path (hidden_r6) -- (missing_24h);
      \path (hidden_r7) -- (implausible);
    \end{scope}
  \end{tikzpicture}
\end{figure}
